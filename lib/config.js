...
async function getConf (argv) {
	...
	if (isWin) {
		...
		const gitForWindows = await reg.query('HKLM/SOFTWARE/GitForWindows').catch(() => ({}));
		if (gitForWindows.installPath) {
			env.GIT_INSTALL_ROOT = gitForWindows.installPath;
		}
		env.Path = [
			'node_modules\\.bin'
		].concat(
			[
				...
				// 'sbin',
				'bin',
				gitForWindows.libexecPath.slice(gitForWindows.installPath.length + 1).replace(/^\\?(.*?\\).*$/, '$1bin')
				// 'usr/games',
				// 'usr/local/games'
			].map(dir => `%GIT_INSTALL_ROOT%/${dir}`),
			[
				'bin',
				'.local/bin'
			].map(dir => `%HOME%/${dir}`),
			...
			'%SYSTEMROOT%',
			'%Path%'
		).filter(Boolean).map(path.win32.normalize).join(';') + ';';
	} else {
	...
	argv.forEach(arg => {
		...
		if (/^(\w+)-mirrors-prefix$/i.test(key)) {
			...
			return;
		} else if (/^(\w+)[-_]proxy$/i.test(key)) {
			env[RegExp.$1.toLowerCase() + '_proxy'] = value;
			return;
	...
	if (isWin && (env.http_proxy == null || env.https_proxy == null)) {
	...
	if (env.http_proxy == null && env.https_proxy) {
		env.http_proxy = env.https_proxy;
	}

	let proxy = env.https_proxy || env.http_proxy;
	if (proxy) {
